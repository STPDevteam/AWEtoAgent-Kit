...
This project was scaffolded with `create-agent-kit` and includes **backend-managed ERC-8004 identity** built on [`@AWEtoAgent/core`](https://www.npmjs.com/package/@AWEtoAgent/core) and [`@AWEtoAgent/identity`](https://www.npmjs.com/package/@AWEtoAgent/identity).

### Features

- ✅ Backend-managed ERC-8004 identity registration (via `createTokenWithIdentity`)
...
Unlike the standard "identity" template, this template **does not** perform client-side ERC-8004 registration. Instead:
...
This approach simplifies the agent setup by delegating identity registration to the backend.
...
- Posted to your backend's `/api/agents/init` endpoint with `registerIdentity: true`
- The backend created the token and registered the ERC-8004 identity
