...
This is a Bun HTTP agent with ERC-8004 identity registration and trust metadata. It combines `@awe-agents/core` for the agent framework and `@awe-agents/identity` for on-chain identity management.
...
> **New:** The CLI now auto-generates an agent wallet (if you leave the field blank) and immediately runs the onboarding flow (ERC-8004 registration + backend `/api/agents/init`). If it fails—or you need to re-run—it also scaffolds `scripts/onboard-agent.ts` and the `bun run agent:onboard` script for you.

**Key Dependencies:**
- `@awe-agents/core` - Agent app framework
- `@awe-agents/identity` - ERC-8004 identity helpers
...
- **Generated by:** `generateAgentMetadata()` from `@awe-agents/identity`
...
AGENT_SHORT_DESCRIPTION=On-chain agent with verifiable identity
...
# Identity metadata
AGENT_METADATA_URI=https://agent.example.com/.well-known/agent-metadata.json
AGENT_CARD_URI=https://agent.example.com/.well-known/agent-card.json
...
2. `AGENT_WALLET_PRIVATE_KEY` is set (or auto-generated) and funded for gas
...
- [@awe-agents/identity docs](../../../identity/README.md)
