...
This is a Bun HTTP agent with ERC-8004 identity registration and trust metadata. It combines `@AWEtoAgent/core` for the agent framework and `@AWEtoAgent/identity` for on-chain identity management.
...
- **Generated by:** `generateAgentMetadata()` from `@AWEtoAgent/identity`
